...
class Game:
    ...
    def __init__(self, bot_1, bot_2, game_size=6, game_config=0):
        ...
        self.out_file = open(
            "matches/match" + str(time.time()).split(".")[0] + ".amazons", "w+"
        )
    ...
    def next_move(self):
        ...
        move = self.players[self.current_move_count % self.player_count].next_move(
            self.base_state
        )
        time_end = time.time()
        ...
        print(
            "Move#"
            + str(self.current_move_count + 1)
            + ": P"
            + str(self.current_move_count % self.player_count + 1)
            + " played '"
            + " ".join(move)
            + "'in "
...
for match_up in itertools.permutations(range(len(bots)), 2):
    g = Game(
        bots[match_up[0]].AmazonsPlayer(1, 2),
        bots[match_up[1]].AmazonsPlayer(2, 1),
        6,
        0,
    )
    res = g.play()
